...
export default function StockOutPage() {
  ...
  async function updateStatus(id: string, status: string) {
    ...
    try {
      if (status === 'issued') {
        // Use the atomic RPC function to deduct stock and mark as issued
        const { error } = await supabase.rpc('issue_voucher_and_deduct_stock', {
          p_voucher_id: id,
          p_issued_by: userProfile!.id,
        })
        if (error) throw error
      } else {
        const update: Record<string, string> = { status }
        if (status === 'approved' && userProfile) update.approved_by = userProfile.id
        const { error } = await supabase.from('issue_vouchers').update(update).eq('id', id)
        if (error) throw error
      }
      await loadVouchers()
    } catch (err: any) {
  ...
  const canIssue =
    ...
    userProfile?.role === 'store_officer'
  ...
  return (
    <div className="animate-in">
          ...
          <button
            onClick={() =>
              exportToCSV(
                ...
                generateExportFilename('vouchers')
              )
            }
      ...
      {/* Action error banner */}
      {actionError && (
        <div className="mb-4 p-3 rounded-lg bg-red-50 text-red-700 text-sm border border-red-200 flex items-center justify-between">
          ...
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="text-center py-12 text-gray-400">
                    <ArrowUpFromLine size={32} className="mx-auto mb-2 opacity-30" />No vouchers found
                  </td>
                </tr>
              ) : (
                vouchers.map(v => {
                  ...
                  return (
                    <tr key={v.id}>
                      <td>
                        <span className="font-mono text-sm font-700 text-amber-700">
                          {v.voucher_number}
                        </span>
                      </td>
                      <td className="font-600 text-gray-800">
                        {(v as any).department?.name ?? '—'}
                      </td>
                      <td className="text-gray-600 text-sm">
                        {(v as any).requested_by_profile?.full_name ?? '—'}
                      </td>
                      <td className="text-gray-600">{formatDate(v.request_date)}</td>
                      <td
                        className="text-gray-700 text-sm max-w-[200px] truncate font-500"
                        title={itemsList}
                      >
                        {itemsList || '—'}
                      </td>
                      <td className="text-gray-500 text-sm max-w-[160px] truncate">
                        {v.purpose ?? '—'}
                      </td>
                      <td className="text-right font-700 text-gray-800">
                      ...
                      <td>
                        <div className="flex justify-end gap-1">
                          ...
                          {v.status === 'draft' && (
                            <button
                              onClick={() => updateStatus(v.id, 'submitted')}
                              disabled={isActing}
                              className="px-2 py-1 rounded text-xs text-white bg-blue-500 hover:bg-blue-600 transition-colors disabled:opacity-50"
                            >
                              {actionLoading === v.id + 'submitted'
                                ? <RefreshCw size={11} className="inline animate-spin" />
                                : 'Submit'}
                            </button>
                          )}
                          ...
                          {v.status === 'approved' && canIssue && (
                            <button
                              onClick={() => updateStatus(v.id, 'issued')}
                              disabled={isActing}
                              className="px-2 py-1 rounded text-xs text-white bg-purple-600 hover:bg-purple-700 transition-colors flex items-center gap-1 disabled:opacity-50"
                            >
                              {actionLoading === v.id + 'issued'
                                ? <RefreshCw size={11} className="inline animate-spin" />
                                : <><Send size={11} /> Issue</>}
                            </button>
                          )}
      ...
      {showModal && (
        <VoucherModal
          ...
          onSave={() => { setShowModal(false); loadVouchers() }}
        />
      )}
      {viewVoucher && (
...
// ---------------------------------------------------------------------------
// VoucherModal — create new voucher
// ---------------------------------------------------------------------------
function VoucherModal({ userProfile, departments, onClose, onSave }: any) {
  ...
  useEffect(() => {
    supabase
      ...
      .select('id,name,item_code,unit_cost,quantity_in_stock,unit:units(abbreviation)')
      .eq('is_active', true)
          ...
          <div>
            ...
            <div className="border border-gray-100 rounded-xl overflow-hidden">
              <table className="data-table">
                    ...
                    return (
                      <tr key={i}>
                        ...
                        <td className="text-right text-sm text-gray-500">
                          {item ? `${item.quantity_in_stock} ${(item as any).unit?.abbreviation ?? ''}` : '—'}
                        </td>
                        <td>
                          <input
                            type="number"
                            ...
                            className="form-input text-sm py-1.5 w-24 text-right"
                            required
                          />
                        </td>
                        <td className="text-right text-sm text-gray-600">
                          {item ? formatCurrency(item.unit_cost) : '—'}
                        </td>
                        <td className="text-right font-700 text-sm">
                          {formatCurrency(Number(line.quantity_requested) * (item?.unit_cost ?? 0))}
                        </td>
                        <td>
                          <input
                            value={line.notes}
                ...
                <tfoot>
                  <tr>
                    <td colSpan={4} className="text-right font-700 text-gray-700 py-3 px-4">
                      Estimated Total:
                    </td>
...
// ---------------------------------------------------------------------------
// VoucherViewModal — read-only detail view
// ---------------------------------------------------------------------------
    ...
    supabase
      ...
      .select(`
        *,
        ...
        requested_by_profile:profiles!issue_vouchers_requested_by_fkey(full_name),
        approved_by_profile:profiles!issue_vouchers_approved_by_fkey(full_name),
        issued_by_profile:profiles!issue_vouchers_issued_by_fkey(full_name),
        items:issue_voucher_items(
          *,
          item:inventory_items(name, item_code, unit:units(abbreviation))
        )
      `)
      .eq('id', voucher.id)
      .single()
          ...
          <div className="p-6 space-y-5">
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <span className="font-600 ml-1">{full.department?.name}</span>
              </div>
              <div>
                <span className="text-gray-500">Status:</span>
                <span className="ml-1"><StatusBadge status={full.status} /></span>
              </div>
              <div>
                <span className="text-gray-500">Requested By:</span>
                <span className="ml-1">{full.requested_by_profile?.full_name}</span>
              </div>
              <div>
                <span className="text-gray-500">Request Date:</span>
                <span className="ml-1">{formatDate(full.request_date)}</span>
              </div>
              {full.approved_by_profile && (
                <div>
                  <span className="text-gray-500">Approved By:</span>
                  <span className="ml-1">{full.approved_by_profile.full_name}</span>
                </div>
              )}
              {full.issued_by_profile && (
                <div>
                  <span className="text-gray-500">Issued By:</span>
                  <span className="ml-1">{full.issued_by_profile.full_name}</span>
                </div>
              )}
              {full.purpose && (
                <div className="col-span-2">
                  <span className="text-gray-500">Purpose:</span>
                  <span className="ml-1">{full.purpose}</span>
                </div>
              )}
            </div>
            ...
            <table className="data-table border border-gray-100 rounded-xl overflow-hidden">
                ...
                {full.items?.map((item: any) => (
                  <tr key={item.id}>
                    ...
                    <td className="text-right">
                      {item.quantity_requested} {item.item?.unit?.abbreviation}
                    </td>
              ...
              <tfoot>
                <tr>
                  <td colSpan={5} className="text-right font-700 text-gray-700 py-3 px-4">
                    Grand Total:
                  </td>
                  <td className="text-right font-700 text-gray-900 py-3 px-4">
                    {formatCurrency(full.total_value)}
                  </td>
                </tr>
              </tfoot>
            </table>
